...
| `src/app/(app)/`                                        | home, `protocols` (catalog; `[id]` detail and Play; `[id]/run` headband pre-flight, run with EEG, upload at finish; `media` your uploads, its own section; `[id]/edit` the builder), recordings (`[id]/review` video + trail + EEG on one clock), neurometrics, account; `/library` and `/record` redirect                                                                                               |
| `src/app/(admin)/admin/`                                | dashboard, `applications` (beta selection board: decide, cohorts; backend V3-0008)                                                                                                                                                                                                                                                                                                                       |
| `src/app/api/backend/[...path]/route.ts`                | BFF proxy: allow-list `auth/`, `recordings`, `admin/`, `sessions`, `media`, `protocols`, `annotations`, `config`, `workspaces`; attaches the cookie JWT; forwards `etag` and `x-next-cursor`                                                                                                                                                                                                             |
...
| `src/components/`                                       | `ui.tsx` primitives, `TrailPlot` (the trail on the session's landscape) and `TrailRibbon` (the trail as one smooth stroke, shared by every surface that draws one), `BallMapperGraph`/`MetricCurve` (Plotly), `SignalPreview` (uPlot), `UploadDialog`, `AppShell` (sidebar, appearance switch), toasts                                                                                                   |
...
| `src/components/review/`                                | scrubber with block bands, trail with a cursor, band-power strip, notes (`/recordings/[id]/review`)                                                                                                                                                                                                                                                                                                      |
