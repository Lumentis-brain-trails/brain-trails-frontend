...
| `src/app/(app)/`                                        | home, `protocols` (catalog; `[id]` detail and Play; `[id]/run` headband pre-flight, run with EEG, upload at finish; `media` your uploads, its own section; `[id]/edit` the builder), recordings (`[id]` opens on the comparison view; `[id]/review` and `/neurometrics` redirect there), account; `/library` and `/record` redirect                                                                      |
| `src/app/(admin)/admin/`                                | dashboard, `applications` (beta selection board: decide, cohorts; backend V3-0008)                                                                                                                                                                                                                                                                                                                       |
| `src/app/api/backend/[...path]/route.ts`                | BFF proxy: allow-list `auth/`, `recordings`, `admin/`, `sessions`, `media`, `protocols`, `annotations`, `config`, `taxonomies`, `workspaces`; attaches the cookie JWT; forwards `etag` and `x-next-cursor`                                                                                                                                                                                               |
...
| `src/lib/trailPath.ts`                                  | the trail's drawn shape: centripetal Catmull-Rom through the windows, cut into the strokes that colour it; the one definition every renderer draws from                                                                                                                                                                                                                                                  |
...
| `src/components/`                                       | `ui.tsx` primitives, `TrailPlot` (the trail on the session's landscape) and `TrailRibbon` (the trail as one smooth stroke, shared by every surface that draws one), `SignalPreview` (uPlot), `UploadDialog`, `AppShell` (sidebar, appearance switch), toasts                                                                                                                                             |
| `src/lib/builder/`                                      | the builder's model: `draft.ts` (pure edits over the tree: insert, move, group, clip widths), `history.ts` (undo/redo as whole drafts), `fields.ts` + `schemas.ts` (the inspector's forms, from the exported block JSON Schema)                                                                                                                                                                          |
| `src/lib/review/timeline.ts`                            | reading a session back: blocks from `block_start`/`block_end`, and the video's own clock from the onsets' `media_time_ms` (S20)                                                                                                                                                                                                                                                                          |
...
| `src/components/builder/`                               | timeline, bin, inspectors, publish dialog (`/protocols/[id]/edit`)                                                                                                                                                                                                                                                                                                                                       |
| `src/components/compare/`, `src/lib/compare/`           | the recording page's comparison view: two blocks side by side (subgrid columns), each with its trail on the session's terrain (labels coloured by what was on screen and marked by what was done), the task repainted from the timeline, and rows the reader adds (band power, dynamics, task scores with the average person; backend V3-0012), each cell blue/yellow when its block is higher/lower     |
