...
| `src/components/`                        | `ui.tsx` primitives, `TrailPlot` (flat/terrain toggle) with `TerrainScene` (react-three-fiber), `BallMapperGraph`/`MetricCurve` (Plotly), `SignalPreview` (uPlot), `UploadDialog`, header, toasts |
