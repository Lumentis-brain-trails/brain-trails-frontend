...
| `src/app/(app)/`                                        | home, `protocols` (catalog; `[id]` detail and Play; `[id]/run` headband pre-flight, run with EEG, upload at finish; `media` your uploads), record, recordings, neurometrics, account; `/library` redirects                         |
| `src/app/(admin)/admin/`                                | dashboard, `applications` (beta selection board: decide, cohorts; backend V3-0008)                                                                                                                                                 |
| `src/app/api/backend/[...path]/route.ts`                | BFF proxy: allow-list `auth/`, `recordings`, `admin/`, `sessions`, `media`, `config`, `workspaces`; attaches the cookie JWT; forwards `etag` and `x-next-cursor`                                                                   |
